...
META=ROOT/"ViralSpawnTV_Longform_V1_2_1_metadata.json"
RESULT=ROOT/"youtube_longform_upload_result_v1_2_1.json"
def main():
    m=json.loads(META.read_text(encoding="utf-8"))
    if m.get("publish_status")!="READY_FOR_UPLOAD": raise RuntimeError("Not upload-ready.")
    if not m.get("continuous_gameplay") or m.get("narration_cards"): raise RuntimeError("Visual safety check failed.")
    ...
    RESULT.write_text(json.dumps({"success":True,"video_id":response["id"],"privacy_status":"public","title":m["title"]},indent=2),encoding="utf-8")
    print("PUBLIC LONG-FORM V1.2.1 UPLOAD:",response["id"])
if __name__=="__main__":
    try: main()
    except Exception as e:
        print("LONGFORM V1.2.1 UPLOAD FAILED:",e);sys.exit(1)
